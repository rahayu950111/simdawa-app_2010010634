...
*/
        ...
        if ($cek->num_rows() > 0) {
            ...
            if ($data_pengguna->keterangan == "Belum Diverifikasi" || $data_pengguna->keterangan == "Akun
Dibatalkan") {
                $return = ['status' => False, 'pesan' => 'Akun anda belum diverifikasi oleh operator
kemahasiswaan'];
            } else {
                $return = ['status' => True, 'data_login' => $data_login, 'nama_lengkap' => $data_pengguna->nama_lengkap];
            }
        } else {
            $return = ['status' => False, 'data_login' => '', 'pesan' => 'Username dan password tidak ditemukan di
sistem!'];
        }
        return $return;
    }
